...
# -----------------------------------------------------------------------------------------------------
# Importieren eigene Module
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__),'src')))
import Module as M

# -----------------------------------------------------------------------------------------------------
# Importieren Freedyn API
sys.path.insert(0, 'C:\\FreeDyn_Release_2024_9\\FreeDynAPI\\fdApi')
import fdApi
...
def main():
    ...
    fdsFilePath = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),'data/EFS2.fds'))

    # Initialisieren FreeDyn API
    fdApi.init()
    ...
    Kraftpfad = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),"data","Kraft.txt"))
    np.savetxt(
        Kraftpfad,
    ...
    #  Modell erstellen, indem der Pfad zur Modelldatei (.fds) übergeben wird
    modelIndex = fdApi.createModel(fdsFilePath,'no')
    #if(modelIndex < 0):
    #    sys.exit()

    # Modell als aktiv setzten (damit modelIndex bei zukünftigen Aufrufen nicht mehr übergeben werden muss)
    ...
    # Simulation starten (Simulations- und Solver-Einstellungen gemäß .fds-Datei)
    ...
    # ++++++++++
    # Alt
    free_dyn_exe = Path(r"C:\FreeDyn2024\bin\FreeDyn.exe")
    free_dyn_model = Path(r"C:\GITHUP_PC3\visdat-course_my\final-assignment\Oliver_Strassl\code\data_2\EFS2")

    command = [str(free_dyn_exe), str(free_dyn_model) + ".fds"]
    ...
    status_file = free_dyn_model.with_suffix(".status")
    # status_file = fdsFilePath.with_suffix(".status")
    ...
    measure_file = free_dyn_model.with_suffix(".mrf")
    ...
    hpfad = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),"data","Uebertragungsfunktion.txt"))
    np.savetxt(
        hpfad,
