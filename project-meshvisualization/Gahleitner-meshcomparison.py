...
def visualize_comparison(original, modified):
    ...
    pl.add_mesh(original,
                scalars='normalized_stress',
                cmap='coolwarm',
                clim=[original['normalized_stress'].min(), original['normalized_stress'].max()],
                scalar_bar_args={"title": "Original normalized Stress"},
                )
    ...
    pl.add_mesh(modified, 
                scalars='normalized_stress', 
                cmap='coolwarm',
                clim=[modified['normalized_stress'].min(), modified['normalized_stress'].max()],
                scalar_bar_args={"title": "Modified normalized Stress"},
                )
    pl.add_text('Modified', font_size=10)
    
    # Difference
    pl.subplot(0, 2)
    pl.add_mesh(meshdiff, 
                scalars='normalized_stress', 
                cmap='coolwarm_r',
                clim=[meshdiff['normalized_stress'].min(), meshdiff['normalized_stress'].max()],
                scalar_bar_args={"title": "Difference normalized Stress"},
                )
