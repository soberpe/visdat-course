...
class StressAnalyzer:
    ...
    def visualize_critical(self, threshold):
        ...
        # Visualize
        pl = pv.Plotter()
        pl.add_mesh(self.mesh, style='wireframe', opacity=0.1, color='grey')
        pl.add_mesh(critical, scalars='S_Mises', cmap='Reds', opacity=0.9)
        pl.add_scalar_bar(title='Critical Stress [MPa]')
        pl.show()

# Usage
analyzer = StressAnalyzer('data/beam_stress.vtu')
analyzer.visualize_critical(threshold=5)
