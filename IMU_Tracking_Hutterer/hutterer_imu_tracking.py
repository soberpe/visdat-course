...
# Datei laden mit zusammengesetztem Namen
accel_df = pd.read_csv(f"IMU_Tracking_Hutterer/data/raw/hutterer_acceleration{suffix}.csv")
gyro_df  = pd.read_csv(f"IMU_Tracking_Hutterer/data/raw/hutterer_gyroscope{suffix}.csv")

# Überprüfen der Bennung der Spalten aus Matlab Mobile
print(accel_df.columns)
...
# Umwandlung der Zeitdaten von Millisekunden (ms) in Sekunden (s)
accel_df['time'] = accel_df['time'] / 1000.0
gyro_df['time']  = gyro_df['time']  / 1000.0

# # Anpassung der Beschleunigungsdaten um einen Offsset
# accel_df['accel_x'] = accel_df['accel_x'] + 0.102836  
...
# Check acceleration units
accel_cols = ['accel_x', 'accel_y', 'accel_z']
max_accel_value = df[accel_cols].abs().quantile(0.95).max()
print(f"Acceleration values appear to be in m/s² (max: {max_accel_value:.2f})")
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/01_raw_sensor_data{suffix}.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/02_filtered_acceleration{suffix}.png', dpi=300)
plt.show()

#-----------------------------------------------------------------------------------------

# Orientation Estimation with Madgwick Algorithm
...
# Transformation der Quaternionen in Euler-Winkel (roll, pitch, yaw)
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/03_orientation_euler{suffix}.png', dpi=300)
plt.show()

#-----------------------------------------------------------------------------------------

# Transformation der Beschleunigungsdaten in das Globale Koordinatensystem
...
# Entfernung der Gravitation aus den globalen Beschleunigungsdaten
...
# Darstellung der berechneten globalen Beschleunigungsdaten ohne Gravitationseinfluss
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/04_global_acceleration{suffix}.png', dpi=300)
plt.show()

#-----------------------------------------------------------------------------------------

# Numerische Integration zur Widerherstellung der Trajektorie
# Integration zur Ermittlung der Geschwindigkeit und Position
...
# Visualisierung der Geschwindigkeitstrajektorie
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/05_velocity{suffix}.png', dpi=300)
plt.show()

#-----------------------------------------------------------------------------------------

# Plot der 2D-Trajektorie (Top-Down View)
...
plt.legend()
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/06_trajectory_2d{suffix}.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/07_trajectory_3d{suffix}.png', dpi=300)
plt.show()

#-----------------------------------------------------------------------------------------

# Berechnen der rekuntruierten Distanz
...
# Advanced: Zero-Velocity Updates (ZUPT) to reduce drift
...
plt.tight_layout()
plt.savefig(f'IMU_Tracking_Hutterer/figures/08_zupt_comparison{suffix}.png', dpi=300)
plt.show()
