...
# Load separate files
accel_df = pd.read_csv('data/raw/Zierler_acceleration.csv')
gyro_df = pd.read_csv('data/raw/Zierler_gyroscope.csv')

# MATLAB Mobile typically uses columns: Time, X, Y, Z
# Rename for consistency
# in excel time = timestamp
...
# accel_df['time'] = accel_df['time'].astype(float) # Datentyp auf float aendern
# gyro_df['time']  = gyro_df['time'].astype(float) # Datentyp auf float aendern
...
print(f"acceleration values appear to be in mm/s^2 (max: {max_accel_value:.2f})")
...
plt.tight_layout()
plt.savefig('figures/01_raw_sensor_data.png', dpi=300)
plt.show()
...
def butter_lowpass_filter(data, cutoff, fs, order=2):
    """Apply a Butterworth low-pass filter tpio the data."""
    nyquist = 0.5 * fs
...
plt.tight_layout()
plt.savefig('figures/02_filtered_acceleration.png', dpi=300)
plt.show()
...
#4 Transform Accelerations to Global Coordinates
...
plt.tight_layout()
plt.savefig('figures/03_orientation_euler.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('figures/04_global_acceleration.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('figures/05_velocity.png', dpi=300)
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig('figures/06_trajectory_2d.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('figures/07_trajectory_3d.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('figures/08_zupt_comparison.png', dpi=300)
plt.show()


from scipy.signal import savgol_filter

#8 Compare three filter types
cutoffs = [3, 5, 10]  # Hz
