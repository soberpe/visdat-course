...
class StressAnalyzer:
    ...
    def calculate_statistics(self):
        ...
        #Maximum allowable stress for safety factor calculation
        ...
        stats = {
            ...
            'std': self.stress.std(),
            'safety_factor': max_allowable / self.stress.max(),
        }
        return stats
    ...
    def visualize_critical(self, threshold):
        ...
        # Visualize
        pl = pv.Plotter()
        pl.add_mesh(self.mesh, opacity=0.3, show_edges=True)
...
print(f"Max Stress: {stats['max']:.2f} MPa")
print(f"Safety factor: {stats['safety_factor']:.2f}")  # Should be > 1.0 for safe!

# Set Threshold for critical stress visualization
threshold = 2.0  # MPa
print(f"Critical cell count: {len(analyzer.get_critical_regions(threshold))}")
analyzer.visualize_critical(threshold)